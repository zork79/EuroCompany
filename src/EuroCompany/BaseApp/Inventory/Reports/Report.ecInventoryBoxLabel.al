...
report 50005 "ecInventory Box Label"
{
    ApplicationArea = All;
    Caption = 'Inventory Box Label';
    Description = 'CS_ACQ_004';
    PreviewMode = PrintLayout;
    RDLCLayout = '.\src\EuroCompany\BaseApp\Inventory\Reports\InventoryBoxLabel.Layout.rdlc';
    UsageCategory = None;
    ...
    dataset
    {
        dataitem(PrintLabel; Integer)
        {
            ...
            dataitem(PageNumber; Integer)
            {
                ...
                column(lblNoOfParcels; lblNoOfParcels) { }
                column(lblTotWeight; lblTotWeight) { }
                column(lblUnitWeight; lblUnitWeight) { }
                ...
                column(WhseDocumentNo; Temp_LabelBuffer_Master."Whse. Document No.") { }
                column(ExternalDocumentNo; Temp_LabelBuffer_Master."External Document No.") { }
            ...
            begin
                ...
                latsImageHelper.CalculateImagePadding(BarcodeB64, 61, 14, BarcodeImagePaddingLeft, BarcodeImagePaddingTop);
                ...
                if Temp_LabelBuffer_Details.FindFirst() then begin
                    ...
                    TotalWeight := 0;
                    UnitWeight := 0;
                    if (Temp_LabelBuffer_Master.Type = Temp_LabelBuffer_Master.Type::Box) then begin
                        if not lawpLogisticUnitInfo.Get(lawpLogisticUnitInfo.Type::Box, Temp_LabelBuffer_Master."No.") then Clear(lawpLogisticUnitInfo);
                    end else begin
                        if not lawpLogisticUnitInfo.Get(lawpLogisticUnitInfo.Type::Pallet, Temp_LabelBuffer_Master."No.") then Clear(lawpLogisticUnitInfo);
                    end;
                    NoOfParcels := lawpLogisticUnitInfo."ecNo. Of Parcels";
                    TotalWeight := lawpLogisticUnitInfo."ecTotal Weight";
                    UnitWeight := lawpLogisticUnitInfo."ecUnit Weight";
                ...
                if (Temp_LabelBuffer_Details.Quantity <> 0) and
                   (lawpGeneralSetup."Print Whse. Label Qty." = lawpGeneralSetup."Print Whse. Label Qty."::Always)
                then begin
                    PrintQty := true;
                end;
            end;
    ...
    var
        ...
        lblReceiptRef: Label 'Receipt ref.:';
        lblOrigin: Label 'Origin:';
        lblNoOfParcels: Label 'No. Parcels:';
        lblTotWeight: Label 'Total Weight:';
        lblUnitWeight: Label 'Unit Weight:';
