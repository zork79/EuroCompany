...
report 50019 "ecShipping Invoice TEST"
{
    ApplicationArea = All;
    Caption = 'Shipping Invoice TEST';
    DefaultLayout = RDLC;
    Description = 'GAP_VEN_001';
    EnableHyperlinks = true;
    PreviewMode = PrintLayout;
    RDLCLayout = '.\src\EuroCompany\BaseApp\Sales\Reports\ShippingInvoiceTEST.Layout.rdlc';
    UsageCategory = None;
    ...
    dataset
    {
        dataitem(SalesHeader; "Sales Header")
        {
            DataItemTableView = sorting("Document Type", "No.")
                                where("Document Type" = const(Invoice), "AltAWPIs Shipping Invoice" = const(true));
            ...
            column(ShippingAgentAddress; ShippingAgent.Address + ', ' + StrSubstNo(ShipAgentText, ShippingAgent."AltAWPPost Code", ShippingAgent.AltAWPCity, ShippingAgent.AltAWPCounty))
            {
            }
            ...
            column(GrosseWight; "AltAWPGross Weight")
            {
            }
                    ...
                    dataitem(RoundLoop; Integer)
                    {
                        ...
                        dataitem(WarehouseEntry; "Warehouse Entry")
                        {
                            ...
                            begin
                                ...
                                SetRange("Whse. Document No.", SalesLine."AltAWPPosted Whse Document No.");
                                SetRange("Whse. Document Line No.", SalesLine."AltAWPPosted Whse Doc. Line");
                        ...
                        begin
                            ...
                            if (Temp_SalesLine_Print.Type = Temp_SalesLine_Print.Type::" ") then begin
                            ...
                            TrackingString := PrintFunctions.GetTrackingString(Temp_SalesLine);

                            if (Temp_SalesLine_Print."APsFOC Attach. to Line No." <> 0) then begin
                                if lSalesLine.Get(Temp_SalesLine_Print."Document Type", Temp_SalesLine_Print."Document No.", Temp_SalesLine_Print."APsFOC Attach. to Line No.") then begin
            ...
            begin
                ...
                if not lCustomer.Get(SalesHeader."Sell-to Customer No.") then Clear(lCustomer);
                ...
                HeaderCommentText := PrintFunctions.GetDocumentCommentHeader(SalesHeader, Report::"ecShipping Invoice TEST");
                PrintFunctions.GetAccompData(SalesHeader, AccompDataCaption_Text, AccompDataValue_Text);
    ...
    var
        ...
        lblTitle: Label 'Shipping Invoice TEST';
        ...
        ShipAgentText: Label '%1, %2, %3', Locked = true;
